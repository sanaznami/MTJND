...
def create_model():
    ...
    MM_model = Model(inputs=base_model.input, outputs=[J1, J2, J3])
    return MM_model

def train_model(MM_model, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs):
...
def test_model(MM_model, X_Test, test_labels, result_path, model_weights_path):
...
def main(base_command, data_dir, checkpoint_path, csv_log_path, result_path, learning_rate, batch_size, epochs, model_weights_path):
    ...
    if base_command == 'train':
        train_model(MM_model, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs)
    elif base_command == 'test':
        test_model(MM_model, X_Test, test_labels, result_path, model_weights_path)
